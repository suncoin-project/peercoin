...
bool AppInit2(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++)
        if (!IsSwitchChar(argv[i][0]) && !(strlen(argv[i]) >= 7 && strncasecmp(argv[i], "bitcoin:", 7) == 0))
            fCommandLine = true;
    ...
    if (mapArgs.count("-proxy"))
    {
        ...
        {
            ThreadSafeMessageBox(_("Invalid -proxy address"), _("BTCcoin"), wxOK | wxMODAL);
            return false;
        }
    }
    ...
    if (mapArgs.count("-reservebalance")) // bitcoin: reserve balance amount
    {
        int64 nReserveBalance = 0;
        if (!ParseMoney(mapArgs["-reservebalance"], nReserveBalance))
        {
            ThreadSafeMessageBox(_("Invalid amount for -reservebalance=<amount>"), _("Bitcoin"), wxOK | wxMODAL);
            return false;
        }
    }

    if (mapArgs.count("-checkpointkey")) // bitcoin: checkpoint master priv key
    {
        if (!Checkpoints::SetCheckpointPrivKey(GetArg("-checkpointkey", "")))
            ThreadSafeMessageBox(_("Unable to sign checkpoint, wrong checkpointkey?\n"), _("Bitcoin"), wxOK | wxMODAL);
    }
