...
// bitcoin: optional setting to unlock wallet for block minting only;
...
// bitcoin: total coins staked (non-spendable until maturity)
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (int i = 0; i < pcoin->vout.size(); i++)
            {
                if (pcoin->nTime > nSpendTime)
                    continue;  // bitcoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // bitcoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // bitcoin: not avatar mode
                    {
...
// bitcoin: create coin stake transaction
...
// bitcoin: check 'spent' consistency between wallet and txindex
// bitcoin: fix wallet spent state according to txindex
...
// bitcoin: disable transaction (only for coinstake)
